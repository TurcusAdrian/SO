...
void verificare_argumente(int argc, char **argv) {
    ...
    if (argc != 4) {
      printf("Usage: %s <directory_input> %s <directory_output> %c <caracter>\n", path_to_directory_input(argv),path_to_directory_output(argv),path_caracter(argv));
        exit(EXIT_FAILURE);
    }
    

    if (!e_director(path_to_directory_input(argv)) || !e_director(path_to_directory_output(argv)) || !isalnum(path_caracter)) {
        perror("Unul din argumentele date nu e director sau nu e caracter!\n");
        exit(EXIT_FAILURE);
    }
}
      ...
      child_count++;
	    continue;
            ...
            if (pid_reg == 0) {
                ...
                if (bytes_read > 0) {
                    total_correct_sentences += atoi(buffer);
                }
